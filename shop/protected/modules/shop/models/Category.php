...
class Category extends CActiveRecord
{
	...
	public static function getStringChilds($id) {
		$str = $id;
		$item = Category::getChildIds(id);		
		return $item;
	}
	...
	public static function getChildIds($id) {
		...
		foreach(Category::model()->findAll('parent_id = ' . $id) as $model) {
			...
			$item = CMap::mergeArray($item,Category::getChildIds($model->category_id)) ;
		}        
	...
	public function attributeLabels()
	{
		return array(
			'category_id' => '#',
			'parent_id' => Yii::t('ShopModule.shop', 'Danh mục cha'),
